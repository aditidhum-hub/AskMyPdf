import { PdfDocument, Citation, ChatMessage } from '../types.js';
...
export async function askDocumentAssistant(
  ...
  try {
    const apiUrl = typeof window !== 'undefined' ? '/api/chat' : 'http://localhost:3000/api/chat';
...
/**
 * Executes cross-document comparative RAG analysis across multiple indexed documents.
 */
export async function askComparativeAssistant(
  question: string,
  documents: PdfDocument[]
): Promise<AssistantResponse> {
  const { InMemoryVectorIndex } = await import('./ragEngine');
  const index = new InMemoryVectorIndex();
  index.indexDocuments(documents);

  const compData = index.retrieveComparativeContext(question, documents, 3);

  const docSummaries = documents
    .map(d => `- **${d.name}** (${d.pageCount} pages): ${d.summary.slice(0, 130)}... [Page 1]`)
    .join('\n');

  const sourcesList = compData.citations.slice(0, 4)
    .map((c, i) => `* **Finding ${i + 1} [${c.heading}]**: "${c.snippet}"`)
    .join('\n\n');

  const content = `### Cross-Document Comparative Analysis
...
Both documents have been retrieved via hybrid semantic embeddings and keyword matching. You can ask follow-up questions comparing specific metrics or technical claims between them.`;

  return {
    content,
    citations: compData.citations,
  };
}
